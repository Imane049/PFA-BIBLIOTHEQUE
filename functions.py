...
def searchRental(searchrental_entry, RentalTree):
    ...
    cursor.execute("SELECT rentalS.*, CLIENT.PERSONAL_NAME || ' ' || CLIENT.FAMILY_NAME, BOOKS.TITLE, rentalS.DATE_EMPRUNT, date('2023-05-07', '+7 days') FROM (rentalS JOIN BOOKS ON rentalS.BOOK_ID=BOOKS.ID) JOIN CLIENT ON rentalS.CLIENT_ID=CLIENT.ID WHERE BOOK_ID LIKE ? OR CLIENT_ID LIKE ? OR CLIENT.PERSONAL_NAME LIKE ? OR CLIENT.FAMILY_NAME LIKE ? OR BOOKS.TITLE LIKE ? OR BOOKS.ISBN LIKE ? OR BOOKS.AUTHOR LIKE ? OR BOOKS.ISBN LIKE ? OR BOOKS.GENRE LIKE ? OR BOOKS.LANGUAGE LIKE ? OR BOOKS.year LIKE ? OR BOOKS.ISBN LIKE ?".format(delai_emprunt, delai_emprunt), ('%'+search_query+'%', '%'+search_query+'%', '%'+search_query+'%', '%'+search_query+'%', '%'+search_query+'%', '%'+search_query+'%', '%'+search_query+'%', '%'+search_query+'%' , '%'+search_query+'%' , '%'+search_query+'%' , '%'+search_query+'%' , '%'+search_query+'%'))
